...
BACKUP_FILE="$TIMESTAMP_DIR/emoji.csv"
: > "$BACKUP_FILE"
echo "name,is_alias,alias_for,url,team_id,user_id,created,is_bad,user_display_name" > $BACKUP_FILE

ALIAS_FILE="$TIMESTAMP_DIR/alias.csv"
: > "$ALIAS_FILE"
echo "name,alias_for" > $ALIAS_FILE

BACKUP_DIR="$TIMESTAMP_DIR/images"
mkdir -p "$BACKUP_DIR"
rm -rf "$BACKUP_DIR"/*

COMMANDS_FILE="$TIMESTAMP_DIR/commands.txt"
: > "$COMMANDS_FILE"
...
while true; do
  ...
  if [[ "$SUCCESS" == "true" ]]; then
    ...
    echo "$RESPONSE" | jq -r '.emoji[] | "\(.name),\(.is_alias),\(.alias_for),\(.url),\(.team_id),\(.user_id),\(.created),\(.is_bad),\(.user_display_name)"' >> $BACKUP_FILE
